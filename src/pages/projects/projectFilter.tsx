...
const Filter: React.FC<IProps> = ({data, filter, onUpdateFilter}) => {
    const programmingLanguages: string[] = getFields(data, "proLanguage") as string[];
        ...
        <div className={projectStyle.filterBar}>
            {
                TreatedLanguages.map((language, index) =>
                    <motion.div
                        ...
                        animate={{
                            backgroundColor: filter === language ? "rgba(222, 222, 222, 1)" : "rgba(222, 222, 222, 0.1)",
                            color: filter === language ? "rgb(27, 27, 27)" : "rgb(222, 222, 222)"
                        }}
                        whileHover={{
